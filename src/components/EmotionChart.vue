...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    // List of emotions to display in the chart
    const topEmotions = ["아주 신나는", "만족스러운", "행복한", "즐거운"];

    const filteredEmotionData = computed(() => {
      return store.state.emotionTags.filter(tag => topEmotions.includes(tag.name)).map(tag => ({
        x: tag.xvalue,
        y: tag.yvalue,
